...
| `python feladat.py vegigjatszas --robusztus` | Ugyanaz, **plusz a teljes robusztussági halmaz (68 eset) a VALÓDI felületen**. Ez mást mér, mint a `golden --halmaz robusztus`: az az értelmezőt (mondat → eszközhívás), ez a teljes utat — orchestrator, ismétlésfigyelés, frusztráció-kiút, magyar mondatgenerálás, próba-napló. | ~5-8 perc | **Kilépőkód 1, ha bármelyik eset KIVÉTELT okozott** — ez a robusztussági elfogadási elv legkeményebb fele. Az összegzés kiírja, melyik eset és milyen kivétel. |
| `python feladat.py riport` | A próba-naplóból **egyetlen, offline megnyitható HTML** (`tools/beszelgetes_riport.py`): fordulónként összecsukható blokk a prompttal, a nyers modellválasszal, a dátumfeloldással, a lépésidőkkel és a kimenő válasszal MINDKÉT módban. | azonnal | Nem "bukik" — jelentés, nem teszt. A `--megnyit` böngészőben nyitja; a felület "Napló megnyitása" gombja ugyanezt teszi. |
...
| 14 | Egy ajánlat után írd be: `a másodikat` (vagy `az utolsó jó lesz`) | Egyből a megerősítés a MÁSODIK időponttal — **modellhívás nélkül** (a naplóban `orchestrator:sorszam`). | Visszakérdezés, vagy egy másik időpont megerősítése. Ez utóbbi a legdrágább hiba a rendszerben. |
| 15 | Keress kétszer egymás után olyan boltban, ahol nincs beosztás (Szundi) | A második válasz KIMONDJA, hogy ott sincs időpont, ÉS felajánlja a kiutat — nem csak a kiutat. | Csak a „így nem jutunk előre" mondat: a kiút elnyelte a tényt. |
