...
- **Tesztek:** 807 zöld + 1 `xfail` (`tests/egyseg/test_alapsema.py::test_cross_org_reference_ma_not_bukik_el`, `strict=True`). A kör 115 új tesztet hozott, két új fájlban: **`test_beszelheto.py`** (a beszélhető mód — benne a TELJES sablonkészletre futó formai vizsgálat: számjegy, kötőjel, zárójel, felsorolásjel, két mondat, egy kérdés) és **`test_robusztus_halmaz.py`** (a robusztussági halmaz determinisztikus védőhálója — a négy biztonsági szám mostantól TESZT, nem csak kézi mérés; plusz a két javított mérési hiba rögzítése).
