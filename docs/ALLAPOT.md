...
1. **Aki nem ismeri a rendszert, azt a rendszer FAGGATJA.** Négy
   forduló alatt jutott el a köszönéstől odáig, hogy menjen be a boltba
   élőben — úgy, hogy keresés egyszer sem futott. Minden rétegünk
   ugyanarra a ki nem mondott feltevésre épült: hogy a vásárló tudja,
   mit lehet nálunk kérni.
2. **A hiányzó kategória nem semlegesen viselkedik.** A köszönés és a
   „milyenek vannak?" a legközelebbi meglévő kategóriába esett — ott
   visszakérdezés, majd kiút lett belőle. A kapuőr ötödik kategóriát
   (`KOSZONES`) és a rendszer hatodik eszközt kapott (`kinalat`),
   mindkettő MODELLHÍVÁS NÉLKÜL. Ugyanaz a minta, mint az ADR-030
   `META_KERDES`-énél.
3. **A feladás csak próbálkozás után értelmes.** A kiút kapuja
   állapotkérdés (`utolso_kereses`), nem szövegkérdés: keresés nélkül a
   katalógus megy ki helyette — de legfeljebb EGYSZER, utána a kiút jön,
   bolt-kérdéssel. A létra négy fokú, és mind a négy fok más.
4. **A bolti tudás a törzsadatból jön, a mondatba ragozva.** „a Szundiba
   altatóért, az Ügyifogyiba petárdáért" — ha az admin átírja a leírást,
   a mondat is változik, kódmódosítás nélkül. A ragozás közelítés
   (`assistant/valasz/ragozas.py`), és a korlátai ki vannak mondva.

**Mérve:** `elso_talalkozas` réteg 100% (8 eset, a négy próbaforduló szó
szerint), beszédhelyzetek 94,9% (39 eset), a nyelvi halmaz 84,1% → 84,1%
— a zajküszöbön belül, tehát **nem javulást állítunk, hanem azt, hogy
nem rontottunk**. A két végigjátszás-hibát (ismétlődő katalógus, hazudós
kiút-mondat) a fej nélküli végigjátszás fogta meg, nem a golden set:
azok nem az értelmezésről szólnak, hanem arról, mit lát a képernyőn a
vásárló.

**A 2026-09-12-i kör négy mondata:**

1. **A szórás ellen a halmaz mérete a gyógyszer, nem a futásszám.** A
   három leggyengébb réteg tíz-tíz új esetet kapott (elengedés 5→15,
   tájszólás 4→14, töredékes 4→14), más beszédhelyzetekből. A billenő
   esetek ARÁNYA 3,6% → 2,4% (a számuk ugyanaz maradt: kettő) — egy
   ötelemű rétegen egyetlen eset 20 pont, tizenötön 6,7.
2. **A futtató mostantól kimondja, ha egy különbség csak szórás.**
   `--ismetles 2` kiírja a futásonkénti tartományt, `--elozo <json>`
   pedig ítéletet mond: 3 százalékpont (vagy a mért tartomány) alatt
   NEM állítható javulás vagy romlás. Az ítélet a JSON-ba is bekerül,
   tehát egy későbbi jelentés sem hivatkozhat rá másképp.
3. **A MINDEGY visszavonható lett** („mégis inkább a nagyot") — de nem
   prompttal: az 2026-09-05-én mérve megbukott. Determinisztikus kapu
   dönti el: ha a mondat kimond egy konkrét értéket, az felülírja a
   MINDEGY-et (ADR-031).
4. **A névmási visszautalás áthozza a kemény részt.** Az „és csütörtökön
   UGYANEZ?" mondatnál a méret is átjön, nem csak a bolt — szűk
   kivétel az ADR-019 szabályán, mert a névmás kimondottan a korábbi
   tartalomra mutat. Névmás nélkül („és csütörtökön?") változatlanul nem
   jön át, és ezt ellenpróba őrzi.

**Mérve** (85 eset, két futás): 84,1% és 86,5%, tartomány 2,4 pont — a
zajküszöbön belül, tehát a korábbi számokhoz képest se javulás, se
romlás nem állítható. A négy javított eset viszont MINDKÉT futáson megy.
Az `elengedes` réteg 47% — továbbra is a leggyengébb, de most 15 eseten
mérve, és a bukások egyfélék: a modell ragaszkodik a korábbi bolthoz.
Részletek: `docs/HALMAZ_BOVITES_20260912.md`.

**Az első éles próba három mondata** (kézi próba, 9 forduló,
`docs/ELES_PROBA_20260905.md`):

1. **Ha egy rendszer nem tud magáról beszélni, akkor foglalni fog
   helyette.** A „csak a választ beszéled?" kérdésből KERESÉS lett, és a
   már lezárt foglalás állapota (`KESZ`) is visszaromlott. A kapuőr
   negyedik kategóriát kapott (`META_KERDES`): a modell meg sem szólal,
   a válasz sablonból jön, az állapot nem mozdul.
2. **Amit a vásárló átad, azt el kell venni.** A „nekem mind jó. válasz
   te." mondatra a rendszer ÚJRA felajánlotta ugyanazt a három
   időpontot — visszaadta a döntést annak, aki épp lemondott róla. Az új
   `dontsd_el_te` irányítási érték a pontozó első jelöltjét veszi, és
   KIMONDOTT időponttal kér megerősítést.
3. **Az első forduló 13,93 s volt, a többi 3,5 s.** A különbség a modell
   betöltése — a felület mostantól indításkor, háttérszálon előmelegít,
   és a golden futtató meg a végigjátszás is.

**A napló 9. fordulója golden eset lett** (`elesbol-01-a-het-tizenotos`)
— és a várt választ EMBER írta be: a rendszer akkor a másodikat
választotta (07:00–07:15), a helyes válasz a harmadik (07:15–07:30). A
golden futtató ehhez bővült: egy eset megadhatja, MIT ajánlottunk fel
(`felajanlott`) — enélkül a jelöltre hivatkozó mondatok mérhetetlenek
voltak (ez az ADR-028-ban még nyitott korlátként szerepelt).

**Mérve:** mindhárom éles eset MEGY (`elesbol` réteg 100%), a
beszédhelyzetek halmaz 85,7% → **90,3%** (31 eset). A nyelvi halmazon a
mozgás (86,4% → 82,7% / 83,6%) a szóráson belül van — a bukó esetek
listája futásról futásra változik, a stabil bukások pedig ismert
korlátok.
...
- **ADR-ek:** 30 dokumentum (001–014, 016–031) + 1 sablon. Ebből **29
